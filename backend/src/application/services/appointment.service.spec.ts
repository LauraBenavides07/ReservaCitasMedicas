...
describe('AppointmentService', () => {
  ...
  describe('create', () => {
    it('debería crear una cita válida', async () => {
      ...
      mockPatientSvc.findByDocumentOrCreate.mockResolvedValue({ id: 'p1', firstName: 'J', lastName: 'P', phone: '123' });
      ...
      expect(mockNotificationSvc.emit).toHaveBeenCalledWith('appointment.created', expect.any(Object));
    });
    ...
    it('debería crear el paciente si no existe', async () => {
      ...
      mockDoctorRepository.findOneBy.mockResolvedValue({ id: 'd1', name: 'Dr. S' });
      mockPatientSvc.findByDocumentOrCreate.mockResolvedValue({ id: 'p1', firstName: 'New', lastName: 'P', phone: '123' });
      mockAppointmentRepository.create.mockReturnValue({ id: 'a1' });
  ...
  describe('cancelAppointment', () => {
    it('debería cancelar una cita con éxito', async () => {
      ...
      expect(mockNotificationSvc.emit).toHaveBeenCalledWith('appointment.cancelled', expect.any(Object));
    });
    ...
    it('debería fallar si el paciente no tiene permiso', async () => {
      mockAppointmentRepository.findOne.mockResolvedValue({
        id: 'a1', patient: { id: 'p1', keycloakId: 'k1' },
  ...
  describe('reschedule', () => {
    it('debería permitir reagendar si hay disponibilidad', async () => {
      const app = {
        id: 'a1', patient: { id: 'p1' }, doctor: { id: 'd1', name: 'D' },
        isOwnedBy: jest.fn().mockReturnValue(true),
        reschedule: jest.fn(),
      };
      mockAppointmentRepository.findOne.mockResolvedValue(app);
  ...
  describe('Confirmación', () => {
    it('confirmAppointment debería cambiar estado a confirmada', async () => {
      ...
      mockAppointmentRepository.save.mockImplementation(a => Promise.resolve({ ...a, status: 'confirmada' }));
